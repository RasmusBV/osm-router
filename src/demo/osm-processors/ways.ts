...
function blocked(way: OSM.ProcessedWay) {
    // Areas
    if(Utils.getTag(way, "area") === "yes") { return false }

    // Toll roads
    //if(Utils.getTag(way, "toll") === "yes") { return false }

    // Public Transit
    const publicServiceVehicle = Utils.getTag(way, "psv")
    if(publicServiceVehicle === "yes" || publicServiceVehicle === "designated") {
        return false
    }
    const bus = Utils.getTag(way, "bus")
    if(bus === "yes" || bus === "designated") {
        return false
    }

    // Steps
    if(profile.avoid.has("steps") && Utils.getTag(way, "highway") === "steps") {
        return false
    }

    // Construction
    if(Utils.getTag(way, "highway") === "construction") { return false }
    if(Utils.getTag(way, "railway") === "construction") { return false }
    const construction = Utils.getTag(way, "construction")
    if(construction && !profile.construction_whitelist.has(construction)) { return false }

    // Proposed
    if(Utils.getTag(way, "proposed") !== undefined) { return false }

    // Reversible oneway
    if(Utils.getTag(way, "oneway") === "reversible") { return false }

    // Impassable
    if(Utils.getTag(way, "impassable") === "yes") { return false }
    if(Utils.getTag(way, "status") === "impassable") { return false }

    return true
}

function avoid(way: OSM.ProcessedWay) {
    const highway = Utils.getTag(way, "highway")
...
function oneway(way: OSM.ProcessedWay) {
    const oneway = Utils.prefixValue(way, profile.restrictions, "oneway") ?? Utils.getTag(way, "oneway")
    if(oneway === "-1") {
        way.innaccessible.forward = true
    } else if(oneway === "yes" || oneway === "1" || oneway === "true") {
        way.innaccessible.backward = true
    } else {
        const junction = Utils.getTag(way, "junction")
        if(
            (Utils.getTag(way, "highway") === "motorway" || 
            junction === "roundabout" ||
            junction === "circular") &&
            oneway !== "no"
        ) {
...
function service(way: OSM.ProcessedWay) {
    const service = Utils.getTag(way, "service")
    if(service && Utils.getTag(way, "foot") === "yes") {
        return false
    }
...
function alternateOptions(way: OSM.ProcessedWay) {
    if((Utils.getTag(way, "route") ?? "") in profile.route_speeds) {
        return false
    }
    return true
}
...
function highOccupancyVehicle(way: OSM.ProcessedWay) {
    const hov = Utils.getTag(way, "hov")
    if(hov === "designated") {
...
function penalties(way: OSM.ProcessedWay) {
    ...
    const width = Utils.convertToMeters(Utils.getTag(way, "width")) ?? Infinity
    let lanes = Infinity
    const parsedLanes = parseInt(Utils.getTag(way, "lanes") ?? "x")
    ...
    const alternatingPenalty = (
        Utils.getTag(way, "oneway") === "alternating"
    ) ? 0.4 : 1

    const sideRoad = Utils.getTag(way, "side_road")
